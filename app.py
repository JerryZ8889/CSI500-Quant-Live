...
# ==========================================
# ⚠️ 核心修复区：暴力清除缓存 + 强制加载 ⚠️
# ==========================================

# 1. 清除 Matplotlib 缓存 (解决"死脑筋"问题)
# 每次应用重启时，强制删掉缓存目录，迫使 Matplotlib 重新扫描字体
cache_dir = os.path.expanduser('~/.cache/matplotlib')
if os.path.exists(cache_dir):
    try:
        shutil.rmtree(cache_dir)
        # st.toast("🧹 已清除字体缓存，正在重载...", icon="🔄") # 调试提示
    except Exception as e:
        pass

# 2. 加载 SimHei.ttf
font_path = './csi500_data/SimHei.ttf' # 确保文件名大小写与 GitHub 文件列表完全一致！
...
try:
    if os.path.exists(font_path):
        ...
        # B. 强制设置全局字体 (不通过 font_name，直接指定 SimHei)
        plt.rcParams['font.family'] = ['SimHei'] 
        plt.rcParams['font.sans-serif'] = ['SimHei'] 
        # C. 再次确认 (调试用，成功后可注释)
        # st.success(f"✅ 成功加载本地字体：{font_path}")
    else:
        st.error(f"❌ 严重错误：找不到字体文件！请确认路径：{font_path}")
        # 保底方案
        plt.rcParams['font.sans-serif'] = ['Noto Sans CJK SC', 'WenQuanYi Micro Hei', 'sans-serif']
except Exception as e:
    st.error(f"❌ 字体加载发生异常: {e}")
    # 保底方案
    plt.rcParams['font.sans-serif'] = ['Noto Sans CJK SC', 'WenQuanYi Micro Hei', 'sans-serif']
